    ...
    def publish(self):
        ...
        while not rospy.is_shutdown():
            if (self.cur_pose is not None) and (self.base_waypoints is not None):
                ...
                next_wp = []
                next_wp_i = self.next_wp(self.cur_pose.pose, self.base_waypoints.waypoints)
                count_wp_red = 0

                if self.is_signal_red == True and self.red_wp_i > next_wp_i:
                     count_wp_red = self.wp_count(next_wp_i, self.red_wp_i+1)
                ...
                if (self.is_signal_red == True) and (0 < count_wp_red < STOP_DIST):
                    ...
                    for cur_wp_i in range(LOOKAHEAD_WPS):
                        next_wp_velocity = self.get_waypoint_velocity(self.base_waypoints.waypoints[next_wp_i])
                        if next_wp_velocity > MAXIMUM_SPEED:
                            next_wp_velocity = MAXIMUM_SPEED
                        next_waypoints.append(waypoints[next_wp_i])
                        self.set_waypoint_velocity(next_waypoints, cur_wp_i, MAXIMUM_SPEED)
